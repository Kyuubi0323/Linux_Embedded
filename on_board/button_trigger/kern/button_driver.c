...
static irqreturn_t gpio_irq_handler(int irq,void *dev_id) 
{
  ...
  gpio_set_value(GPIO_OUT, led_toggle);                      // toggle the GPIO_21_OUT
  pr_info("Interrupt Occurred : GPIO_31_OUT : %d ",gpio_get_value(GPIO_OUT));
                ...
                char __user *buf, size_t len, loff_t *off)
{
  ...
  pr_info("Read function : GPIO_31 = %d \n", gpio_state);
                ...
                const char __user *buf, size_t len, loff_t *off)
{
  ...
  pr_info("Write Function : GPIO_31 Set = %c\n", rec_buf[0]);
...
static int __init etx_driver_init(void)
{
  ...
  //Requesting the GPIO
  if(gpio_request(GPIO_OUT,"GPIO_31_OUT") < 0){
  ...
  //Requesting the GPIO
  if(gpio_request(GPIO_IN,"GPIO_30_IN") < 0){
    pr_err("ERROR: GPIO %d request\n", GPIO_IN);
    goto r_gpio_in;
  }
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Hadilao <khoi.nv0323.work@gmail.com>");
